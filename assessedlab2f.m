...
%Defines the RGB values of 9 different lines.
...
%ensures lines with the same value of t have the same colour by forcing the
%colour order corresponding to the colors defined above.
...
%diffusion term
f = [1;2] .*DuDx;
s = [u(1).*(1-u(1)-(0.6.*u(2)));1.5 .*u(2) .* (1-u(2)-(0.7.*u(1)))];
end
